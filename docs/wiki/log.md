...
| 2026-09-26 | feat | 判决层接入:进展页 `#forge` 增「内部设计判决层」一栏 —— 上游闭式解(PF3/FC1/FC4/FC11b/TD19–TD21)实现成六道门,同一份参考设计的结论:MATBG (N=2) 面外 0.12 T ⟹ **无解**(χ_μ = 2.1e−4);面内 1.6 T ⟹ 窗口开(1.479)但线圈峰场 3.416 T 超天花板 ⟹ 不成立 | 需方「Forge 要根据 ProjectionPhysics 内部的设计来实现,然后把进展页的概念改动」;上游 = `logos-42/Hibs-Physics` |
| 2026-09-26 | design | 阶段口径改准:「概念」→ 事实陈述(装置侧还没有装置,当前处在**判据规范**这一步;设计侧已有可运行系统)。门数 16 → 17(新增 G17 内部设计锚点门),首页 / 进展页 / 关于页三处同步 | 「概念」是含糊词;上游那一步有名字。公开纪律:上游 wiki 58 页全 `audience: self` ⟹ 判决漏斗的排期 / 人·月 / 编成 / 门表**不进公开站**,只发这一步的名字 |
| 2026-09-26 | fix | 修站点自己的一个**假红**:`deploy.sh` 的线上验收在 `set -o pipefail` + `set -e` 下,发布刚落地时的抖动会让**整个部署退出 1**(内容其实已核对)。改为重试到判据(最多 3 次),三次都不过才报「这是真红,不是抖动」 | 实测:部署那次退出 1;同一份线上验收独立重跑 **226/226 通过**。后续两次部署把抖动率量出来了:其中一次尝试 1、2 连续失败、**第 3 次才过**(重试把原本的假红救回来了);另一次第 1 次就过 ⇒ 抖动只在发布刚落地那几秒,且**一次重试不够** —— 假红比没有门更伤(会让「部署成功」这个判断失去意义) |
| 2026-09-26 | feat | **公开判决漏斗**:进展页新增「判决漏斗」一节 —— 8 道门表(G0–G7:月 / 交付物 / 通过条件 / 人数 / 预写失败处置)+ 判决期 **15 人·月 = 五年 1.29%**(前 18 个月 93 人·月 = 8.0%,全周期 60 月)+ **生死门 G1(M6)** + μ 数量级阶梯(~1e−4 → 1e−3 → 1e−2 → 1e−1) | 需方决定公开(原页 `audience: self`);页内标注了这次改档:「原标记 audience: self,经需方 2026-09-26 决定公开」。表尾同时写明「数目字是排程假设,不是承诺;货币成本按约定不入账」 |
| 2026-09-26 | feat | `docs.html` 挂 `hushfusion-forge/docs/design-layer.md`(冻结契约);进展页 `#forge` 同步加「判决层契约(冻结文本) →」 | 需方「要挂」。契约本身不含内部排期,可公开 |
| 2026-09-26 | test | `node scripts/verify-site.mjs` 226/226(本地) → 部署后线上验收 226/226;线上读回逐条(内部设计判决层 / 17 道验收门 / 165 步 / χ_μ = 2.1e−4 均在;旧口径「概念阶段」「16 道验收门」全部归零) | 站点自身的门 |

...
| 2026-09-23 | chore | 容器名「账本」→「目录」(需方手改 + 我方补齐 `data-en=Contents`、导语、页级出处、meta;`<title>` 不写数目) | 导语同时去罗列化 |
...
| 2026-09-26 | design | 前门减法:删掉与下方清单逐字重复的滚动展示栏、重复的第二条 `lede`、同页重复的第二个 CTA、空 `figure`/`figcaption` | 需方「内容太乱,用乔布斯技能重新删减」;诊断 = 同一句话在一屏内说了八遍 |
| 2026-09-26 | design | 首页五个支点 → 两条公开技术方向(另三个是动机与愿景),完整五条留在关于页并给回链 | 首页该说"做什么",不该复述海报 |
| 2026-09-26 | feat | 进展页从占位符改成实物:删掉「关键指标 · 数值待项目方提供」空盒子与「合作与资助 · 待项目方提供」占位块;新增 `#forge` 一节(12,001 次评估 / 1.3814 vs −0.2906 / 16 道门 / 4.2 mm 不可造限制 / 一行复现命令) | 唯一有实测数字的东西此前不在站上 |
| 2026-09-26 | feat | 首次接入 FORGE(= `hushfusion-forge`):首页 `#forge` + 进展页 `#forge` + 文档区代码仓库第三行 + 关于页时间线第二个已发生节点 2026-09-26 | 需方「前端设计更新后,把这个项目加进去介绍」 |
| 2026-09-26 | test | `node scripts/verify-site.mjs` 226/226(本地) → `scripts/deploy.sh` 部署后对线上重跑 226/226 | 站点自身的门;线上截图落临时目录,不覆盖仓库证据 |
| 2026-09-26 | chore | 线上读回踩坑记录:Cloudflare Pages 把 `*.html` 308 重定向到无扩展名路径,`curl` 不带 `-L` 会拿到 0 字节 —— 看起来像"没部署",实际是没跟随重定向 | 下次核验线上内容直接查 `/progress`、`/docs` |
| 2026-09-26 | fix | 修 CI 连续三天全红:`untracked_raw_check` 报两个文件未登记 ——(1) `assets/art/mu-dynamics.png`(2026-09-24 新增第 06 册时漏登记);(2) `docs/screenshots/docs.png`(理论页由 `theory.html` 改名后截图名随之变化,而 manifest 里那一行还是 `theory.png`)。按本仓惯例补登 `manifests/raw_sources.csv` 两行 + `docs/ASSETS.md §九` 一行(含 sha256 / 尺寸 / 字节 / 源路径) | 近 4 次 push(2026-09-23 起)CI 全红;同一失败模式 2026-09-22 已出现过一次 —— 页面改名/加册时最容易漏登记 |

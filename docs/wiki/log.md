...
| 2026-09-26 | fix | 修站点自己的一个**假红**:`deploy.sh` 的线上验收在 `set -o pipefail` + `set -e` 下,发布刚落地时的抖动会让**整个部署退出 1**(内容其实已核对)。改为重试到判据(最多 3 次),三次都不过才报「这是真红,不是抖动」 | 实测:部署那次退出 1;同一份线上验收独立重跑 **226/226 通过** —— 假红比没有门更伤(会让「部署成功」这个判断失去意义) |
